...
function photo(img = null, description = "", bgcolor = "lightgray") {
    ...
    p.start = function() {
        ...
        p.constructor.prototype.start.call(this);
    }
    return p;
}
...
t_next.starts(t_next, _ => {
    photos[photo_index].stop();
    if (++photo_index >= photos.length) photo_index = 0;
    localStorage.setItem("dev_photo_index", photo_index);
    photos[photo_index].start();
    description_div.innerHTML = photos[photo_index].description;
    document.body.style.backgroundColor = photos[photo_index].bgcolor;
});
