...
const r_left_entrance        = rect_left(entrance);
...
const r_middle_entrance      = rect_middle(entrance);
...
const t_left_entrance        = touch(r_left_entrance);
...
const t_middle_entrance      = touch(r_middle_entrance);
...
const t_right_entrance       = touch(r_right_entrance);
...
const touches_entrance           = [t_left_entrance, t_middle_entrance, t_right_entrance];
...
const touches_text_entrance      = [t_back_entrance, t_left_entrance, t_text_entrance, t_right_entrance];
...
t_left_entrance.stops(entrance, c_entrance_0, text_entrance);
//t_middle_palace
...
t_text_entrance.stops(c_entrance_0, text_entrance);
...
t_back_entrance.starts(blop, close_back_entrance);

t_left_palace.starts(entrance, touches_entrance);
t_left_san_francisco.starts(palace, touches_palace);
t_left_india.starts(san_francisco, touches_san_francisco);
t_left_entrance.starts(india, touches_india);
...
close_back_entrance.starts(delay(.5).starts(_ => { 
    setTimeout(_ => {
        text_entrance.stop();
        start_start_sets(touches_entrance);
    }, 500);
    window.location = '../'; 
}));
...
if (state.get("src") === i_san_francisco.src) {
    ...
    india.start();
    start_start_sets(touches_india);
} else if (state.get("src") === i_entrance.src) {
    entrance.start();
    start_start_sets(touches_entrance);
} else {
    palace.start();
